...
def create_user(uname="", pw=None, email="", auth=AUTH_TYPE.LOCAL):
    ...
    if not (
        ...
        or not isinstance(pw, (type(None), str))
        or not isinstance(auth, AUTH_TYPE)
    ...
    if (pw != None and auth != AUTH_TYPE.AD) and (
        len(pw) < LoginConfig.password_min_len
    ):
    ...
    with db_session:
        ...
        if user is not None:
            ...
            user_to_return = User(
                ...
                auth_type=auth,
                email=email,
                activated=LoginConfig.auto_activate_accounts,
            )
        ...
        if requested_user is None:
            ...
            if AD_Config.login and uname.endswith(AD_Config.suffix):
                ...
                if is_in_group:
                    ...
                    try:
                        success, created_user = create_user(
                            f"{uname}{AD_Config.suffix}", auth=AUTH_TYPE.AD
                        )
...
"""
        ...
        if token is None:
            ...
            if token_type is Auth_Token:
                token = Auth_Token(
                    user=user, token=token_hex, ip=ip, valid_until=valid_until
                )
            elif token_type is ActivationCode:
